use crate::effects_builders::{EffectDef, to_stereo, EffectParams};
...
use crate::patch_builder::{ParamDefault, ParamInfo, ParamType};
...
impl EffectParams for ReverbParams {
    fn from_table(table: &Table) -> Self {
        Self {
            room_size: table.get("room_size").and_then(|v| v.as_float()).unwrap_or(0.8) as f32,
            damping:   table.get("damping").and_then(|v| v.as_float()).unwrap_or(0.5) as f32,
            length:    table.get("length").and_then(|v| v.as_float()).unwrap_or(0.5) as f32,
        }
    }
